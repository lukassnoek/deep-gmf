...
def CORnet_Z(input_shape=(256, 256, 3), n_classes=4):
    ...
    x = Conv2D(64, 7, 2, activation='relu', padding='same', name='layer-1_conv')(s)
    x = MaxPooling2D(3, strides=2, padding='same', name='layer-1_pool')(x)

    # V2: 128 filters, kernel size 3x3, stride 1
    # 56 x 56 (conv) -> 56 x 56 (maxpool) -> 28 x 28
    x = Conv2D(128, 3, 1, activation='relu', padding='same', name='layer-2_conv')(x)
    x = MaxPooling2D(3, strides=2, padding='same', name='layer-2_pool')(x)

    # V4: 256 filters, kernel size 3x3, stride 1
    # 28 x 28 (conv) -> 28 x 28 (maxpool) -> 14 x 14
    x = Conv2D(256, 3, 1, activation='relu', padding='same', name='layer-3_conv')(x)
    x = MaxPooling2D(3, strides=2, padding='same', name='layer-3_pool')(x)
    
    # IT: 512 filters, kernel size 3x3
    # 14 x 14 (conv) -> 14 x 14 (maxpool) -> 7 x 7
    x = Conv2D(512, 3, 1, activation='relu', padding='same', name='layer-4_conv')(x)
    x = MaxPooling2D(3, strides=2, padding='same', name='layer-4_pool')(x)
