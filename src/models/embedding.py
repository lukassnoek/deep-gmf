...
def _incremental_mean_and_var(X, last_mean, last_variance, last_sample_count, new_sample_count):
    ...
    correction = tf.reduce_sum(X - T, axis=0)
    new_unnormalized_variance = tf.reduce_sum((X - T) ** 2, axis=0)
...
if __name__ == '__main__':
    ...
    N = 4096   # total nr of samples
    K = 100000  # number of features
    batch_size = 1024
    n_comp = 1000 # number of components to keep
    ...
    pca_sk = IncrementalPCA(n_components=n_comp)
