...
def main(model_path, n_comp, n_batches, batch_size, gpu_id, cpu, binocular):
    ...
    layers2analyze = ['input', 'conv', 'globalpool', 'id']    
    ...
    for layer in pbar:
        ...
        for i, (X, _) in enumerate(train_data):
            pbar.set_description(f"{layer.name}, {i+1}/{n_batches}")
