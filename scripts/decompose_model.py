...
@click.option('--n-samples', default=16384)
@click.option('--n-id-test', type=click.INT, default=None)
@click.option('--gpu-id', default=1)
def main(model_path, compress, batch_size, n_samples, n_id_test, gpu_id):
    ...
    with tf.device(f'/gpu:{gpu_id}'):
        ...
        for _, shape_, tex_, bg_ in tqdm(dataset_tf.as_numpy_iterator(), desc='a_F', total=n_batches):
            shape.append(shape_)
            tex.append(tex_)
            bg.append(bg_)
        ...
        for layer in tqdm(layers, desc='Lay'):
            ...
            a_N = []
            for X_, _, _, _ in tqdm(dataset_tf):
            ...
            grp.create_dataset(layer.name, data=a_N.copy())
