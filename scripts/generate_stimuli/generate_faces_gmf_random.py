...
for i in tqdm(range(100)):
    ...
    f_out = str(OUT_DIR / f'id-{str(current_id).zfill(3)}_gender-{gender}_ethn-{ethn}_age-{age}')
    ...
    for ii in range(1):
        xr = round(random.uniform(*XRS), 2)
        yr = round(random.uniform(*YRS), 2)
        zr = round(random.uniform(*ZRS), 2)
        xt = round(random.uniform(*XTS), 2)
        yt = round(random.uniform(*YTS), 2)
        zt = round(random.uniform(*ZTS), 2)
        lx = round(random.uniform(*XYZ_LIGHTS[0]), 2)
        ly = round(random.uniform(*XYZ_LIGHTS[1]), 2)
        ...
        with h5py.File(f_out + '_features.h5', 'w') as f_out:
            ...
            f_out.create_dataset('img', data=img_arr, compression='lzf')
            f_out.create_dataset('shape', data=v_coeff, compression='lzf')
            f_out.create_dataset('text', data=t_coeff, compression='lzf')            
